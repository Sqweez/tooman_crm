                ...
                <v-simple-table v-slot:default>
                    <template>
                        ...
                        <tr v-for="(item, index) of cart" :key="`product-id-${item.uuid}`">
                            ...
                            <td>
                                {{ item.product_price * item.count - (!isPricesWasChangedManually ? (Math.max(discountPercent, item.discount) / 100 * item.product_price * item.count) : 0)| priceFilters }}
                            </td>
                            <td>
...
export default {
            ...
            if (value) {
                ...
                let products = productIds
                    .map(id => {
                        ...
                        return {
                            ...product,
                            initial_price: product.product_price,
                            product_price: this.report.products.find(p => p.product_id === id).product_price,
    ...
    methods: {
        ...
        async onSubmit() {
            ...
            this.$toast.success('Продажа подтверждена');
            this.$emit('cancel');
        },
        getFiltered(e) {
    ...
    computed: {
        ...
        isPricesWasChangedManually () {
            return this.cart.some(c => c.product_price !== c.initial_price);
        },
        discount() {
            if (this.isPricesWasChangedManually) {
                return 0;
            }
