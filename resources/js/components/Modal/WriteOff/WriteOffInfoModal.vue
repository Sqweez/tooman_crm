<template>
    <base-modal :state="state" title="Информация о списании" @cancel="$emit('cancel')" max-width="1500">
        <template #default>
            <v-simple-table v-slot:default class="mt-5">
                <template>
                    ...
                    <tbody>
                    <tr v-for="(item, index) of $store.getters.WRITE_OFF.products" :key="item.id">
                        <td>{{ index + 1 }}</td>
                        <td>
...
<!--                            <v-text-field
                        ...
                        <td>{{ (item.product_price * item.quantity) | priceFilters }}</td>
                    </tr>
                    </tbody>
                </template>
            </v-simple-table>
