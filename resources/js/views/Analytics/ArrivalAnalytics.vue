    ...
    export default {
                ...
                tooltips: {
                    callbacks: {
                        label: function (tooltipItem, data) {
                            return `Поступления: ${new Intl.NumberFormat('ru-RU').format(Math.ceil(tooltipItem.yLabel))}₸`
                        }
                    }
                },
                scales: {
        ...
        methods: {
            ...
            async getSaleAnalytics() {
                ...
                await this.$store.dispatch(ACTIONS.GET_ARRIVAL_ANALYTICS, {
                    start: `${this.start}-01`,
        ...
        computed: {
            ...mapGetters(['ARRIVAL_ANALYTICS', 'ARRIVAL_ANALYTICS_LABELS'])
        },
        components: {
            ArrivalChart
        },
        watch: {
            ARRIVAL_ANALYTICS(val) {
                this.chartData = {
                    labels: [
                        ...this.ARRIVAL_ANALYTICS_LABELS
                    ],
                    datasets: [
                        {
                            label: 'Поступления',
