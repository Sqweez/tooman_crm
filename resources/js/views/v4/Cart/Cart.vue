...
export default {
    ...
    computed: {
        quantities () {
            return this.$store.getters.ALL_QUANTITIES;
        },
        products() {
            let products = this.$store.getters.SEARCHED_PRODUCTS;
    ...
    methods: {
        ...
        backendSearch: debounce(async function (value) {
            if (value.length >= 3 && !this.loading) {
                this.loading = true;
                await this.$store.dispatch('SEARCH_PRODUCTS', value);
                this.loading = false;
            }
        }, 1500),
